...
const InspectPrint = ({jenisProses}) => {
    ...
    useEffect(() => {
        ...
        try {
            const url = jenisProses === "mkl-bj" ? "inspecting/get-inspecting-mkl-bj" : "inspecting/get-inspecting";
            ...
            <div className="card-body print:p-0 landscape-page" ref={printRef} >
                ...
                <tbody>
                  {kartuProsesItem.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>-</td>
                      <td>{inspectItem.filter(insItem => insItem.grade === 4 && insItem.stock_id === item.stock_id).reduce((total, insItem) => total + parseInt(insItem.qty, 10), 0)}</td>
                      <td>{inspectItem.filter(insItem => insItem.grade === 1 && insItem.stock_id === item.stock_id).reduce((total, insItem) => total + parseInt(insItem.qty, 10), 0)}</td>
                      <td>{inspectItem.filter(insItem => insItem.grade === 8 && insItem.stock_id === item.stock_id).reduce((total, insItem) => total + parseInt(insItem.qty, 10), 0)}</td>
                      <td>{inspectItem.filter(insItem => insItem.grade === 7 && insItem.stock_id === item.stock_id).reduce((total, insItem) => total + parseInt(insItem.qty, 10), 0)}</td>
                      <td>{inspectItem.filter(insItem => insItem.grade === 2 && insItem.stock_id === item.stock_id).reduce((total, insItem) => total + parseInt(insItem.qty, 10), 0)}</td>
                      <td>{inspectItem.filter(insItem => insItem.grade === 3 && insItem.stock_id === item.stock_id).reduce((total, insItem) => total + parseInt(insItem.qty, 10), 0)}</td>
                      <td>{inspectItem.filter(insItem => insItem.stock_id === item.stock_id).reduce((total, insItem) => total + parseInt(insItem.qty, 10), 0)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                    <tr>
                        ...
                        <td>PCS</td>
                        <td>{kartuProsesItem.reduce((total, item) => total + item.panjang_m, 0)}</td>
                        <td colSpan={10}>
                            <Stack direction="horizontal" style={{ width: "100%" }}>
                                <div style={{ display: "flex", alignItems: "center", width: "50%" }}>
                                    <div style={{ width: "25px", height: "20px", border:"solid", marginRight: "5px" }}></div>
                                    <div><b>SINGLE FOLDED</b></div>
                                </div>
                                <div style={{ display: "flex", alignItems: "center", width: "50%" }}>
                                    <div style={{ width: "25px", height: "20px", border:"solid", marginRight: "5px" }}></div>
                                    <div><b>DOUBLE FOLDED</b></div>
                                </div>
                            </Stack>
                        </td>
                        <td></td>
                        <td>{inspectItem.filter(item => item.grade === 4).reduce((total, item) => total + parseInt(item.qty, 10), 0)}</td>
                        <td>{inspectItem.filter(item => item.grade === 1).reduce((total, item) => total + parseInt(item.qty, 10), 0)}</td>
                        <td>{inspectItem.filter(item => item.grade === 8).reduce((total, item) => total + parseInt(item.qty, 10), 0)}</td>
                        <td>{inspectItem.filter(item => item.grade === 7).reduce((total, item) => total + parseInt(item.qty, 10), 0)}</td>
                        <td>{inspectItem.filter(item => item.grade === 2).reduce((total, item) => total + parseInt(item.qty, 10), 0)}</td>
                        <td>{inspectItem.filter(item => item.grade === 3).reduce((total, item) => total + parseInt(item.qty, 10), 0)}</td>
                        <td className="fw-bold">
                            {inspectItem.reduce((total, item) => total + (parseInt(item.qty_bit, 10) || 0) + parseInt(item.qty, 10), 0)}
                        </td>
                    </tr>
                </tfoot>
              </Table>
              <Table bordered style={{ fontSize: "9px", borderColor: "black" }}>
                <tbody>
                    ...
                    <tr>
                        <td>CONTOH</td>
                        <td>{inspectItem.filter(item => item.grade === 5).reduce((total, item) => total + parseInt(item.qty, 10), 0)}</td>
                    </tr>
                </tbody>
              </Table>
            </div>
