      ...
      try {
        const endpoint =
          props.jenisProses === "dyeing"
            ? "kartu-proses-dyeing/get-kartu-proses-dyeing/{id}"
            : "kartu-proses-printing/get-kartu-proses-printing/{id}";
        const response = await axiosInstance.get(
          endpoint.replace(
            "{id}",
            props.inspectingData?.kartu_process_dyeing_id ||
              props.inspectingData?.kartu_process_printing_id
          )
        );
        console.log("INSPECTING ITEM", response.data.data);

        const data =
          response.data.data[`kartu_proses_${props.jenisProses}_item`];
        const options = data.map((item) => ({
          value: item.stock_id,
          label: `(${item.panjang_m})  ${
            item.mesin !== null ? " | " + item.mesin : ""
          }  |  ${item.date}`,
          name: "stock_id",
        }));
            ...
            <Form onSubmit={handleSubmit}>
              {props.jenisProses !== "mkl-bj" && (
                <Form.Group className="mb-3">
                  <Form.Label>
                    <strong>Asal Greige</strong>
                  </Form.Label>
                  <CustomSelect
                    options={kartuProsesItemOption}
              ...
              <Row className="mb-3">
                ...
                <Form.Group as={Col} xs={4}>
                  ...
                  <Form.Control
                    type="text"
                    name="join_piece"
                    value={formData.join_piece.toUpperCase()}
                    className="border-bold"
                    onChange={(e) => {
