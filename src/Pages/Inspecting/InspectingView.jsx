...
import { Card, Container, Table, Row, Col, Form, Stack ,Button,Modal } from 'react-bootstrap';
...
const InspectingView = (props) => {
    ...
    const fetchData = useCallback(async () => {
        try {
            if(props.jenisProses !== 'mkl-bj'){
                const response = await axiosInstance.get(`inspecting/get-inspecting/${idInspecting}`);
                ...
                setForm((prevForm) => ({
                    ...
                    kartu_proses_dyeing_id :response.data.data.kartu_process_dyeing_id ||null,
                    kartu_proses_printing_id : response.data.data.kartu_process_printing_id || null,
                    wo_color_id : response.data.data.kartu_process_dyeing?.wo_color_id || response.data.data.kartu_process_printing?.wo_color_id || null,
                    inspection_table: response.data.data.inspection_table
                    
                }));
                setWoColorsOptions([{ value: null, label: response.data.data.kombinasi }]);
                console.log(response.data.data);

            }else{
                const response = await axiosInstance.get(`inspecting/get-inspecting-mkl-bj/${idInspecting}`);
                setData(response.data.data);
                ...
                setWoColorsOptions([{ value: response.data.data.wo_color_id, label: response.data.data.wo_color.mo_color.color }]);
                console.log(response.data.data);
    ...
    const handleSubmitEditHeader = (e) => {
        e.preventDefault();
        const url = props.jenisProses === 'mkl-bj' ? 'inspecting/update-inspecting-mklbj' : 'inspecting/update-inspecting';
        axiosInstance.put(`${url}/${idInspecting}`, form)
        .then(response => {
            setAlertMessage({ show : true, success : true, message : response.data.message });
            console.log(response.data);
            fetchData();
        })
        .catch(error => {
            setAlertMessage({ show : true, success : false, message : 'Tak dapat merubah data saat ini, coba beberapa saat lagi.' });
            console.log(error);
    ...
    const handleWoChange = (selectedOption) => {
        if (selectedOption) {
            const { value, label, warna, mo_id, sc_id, sc_greige_id } = selectedOption;
            setForm((prevForm) => ({
                ...prevForm,
                wo_id: value,
                no_wo: label,
    ...
    const handleFormConfirm = async (confirmed) => {
        ...
        if (confirmed) {    
            try {
                await axiosInstance.delete(`inspecting/delete-inspecting-item/${inspectItemWillDeleted}`);
                closeModal();
                ...
                <>
                    <Container fluid className="p-4">
                        <Card className="mb-3 bg-burgundy-gradient bg-pattern-card py-3 px-4 text-white shadow">
                            <h3 className="mb-0"> {props.jenisProses.toUpperCase()} : {props.jenisProses === 'dyeing' ? data?.kartu_process_dyeing?.no || '-' : data?.kartu_process_printing?.no || '-'}</h3>
                        </Card>
                        <Card className="p-2 shadow-sm">
                            <Stack direction='horizontal' gap={2} className="p-2 ms-auto">
                                <Link to={`/print/inspecting-kartu/${props.jenisProses}/${idInspecting}`}>
                                    <Button className='bg-success text-white' size='sm'><FiPrinter />Print GAP-FRM-GG- 03</Button>
                                </Link>
                                <Link to={`/print/inspecting/${props.jenisProses}/${idInspecting}`}>
                                    <Button className='bg-burgundy text-white' size='sm'><FiPrinter />Print GAP-FRM-VP 03</Button>
                                </Link>
                                <Link to={`/print/fir/${props.jenisProses}/${idInspecting}`}>
                                    <Button className='bg-info text-white' size='sm'><FiPrinter />Print FIR-FRM </Button>
                                </Link>
                                <Button className='bg-burgundy-gradient text-white' size='sm' onClick={() => setShowModal(true)}><FaEdit /> Edit</Button>
                            </Stack>
                                        ...
                                        <tbody>
                                            {props.jenisProses !== 'mkl-bj' && (
                                                <tr>
                                                    ...
                                                    <td>{data?.kartu_process_dyeing?.no || data?.kartu_process_printing?.no || '-'}</td>
                                                </tr>
                                            )}
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <Stack direction="horizontal" gap={3}>
                                                        <div className="text-center w-100">{data.no_lot || '-' }</div>
                                                    </Stack>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                <Stack direction="horizontal" gap={3}>
                                                    <div className="text-center w-100">{units[data.unit || data.satuan] || '-' }</div>
                                                </Stack>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                <Stack direction="horizontal" gap={3}>
                                                    <div className="text-center w-100">{data?.inspection_table || '-' }</div>
                                                </Stack>
                                                </td>
                                            </tr>
                                            {props.jenisProses === 'mkl-bj' && (
                                ...
                                <Col lg={6} md={6} sm={6}>
                                    <Table bordered responsive striped className="small-text m-0">
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><strong>Tanggal Inspect</strong></td>
                                                <td>{new Date(data.date || data.tgl_inspeksi).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) || '-'}</td>
                                            </tr>
                                            <tr>
                    ...
                    <Card className='p-3 my-3 rounded-0 shadow-sm'>
                        <Table bordered responsive striped className="small-text text-center m-0">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                {inspectItem?.map((result, index) => (
                                    <React.Fragment key={index}>
                                        <tr key={index} >
                                            <td style={{ width: '10%' }}>{index + 1}{result.join_piece}</td>
                                            <td>{result.qty}</td>
                                            <td>{grades[result.grade]}</td> 
                                            <td>
                                                {result.defect_item.length > 0 ? (
                                                    result.defect_item.map((defect, defectIndex) => (
                                                        <span key={defectIndex} className={`d-block p-1 ${defectIndex === result.defect_item.length - 1 ? '' : 'border-bottom-bold'}`}>
                                                            {defect.meterage} / {defect.mst_kode_defect.no_urut} / {defect.point}
                                                        </span>
                                                    ))
                                                ) : <span className="text-danger"><i>No Defect</i></span>}
                                            </td>
                                            <td>
                                                <Button 
                                                    variant="warning" 
                                        ...
                                        {
                                            visibleCard.id === result.id && (
                                                <InspectResultEdit result={result} jenisProses={props.jenisProses} closeModal={closeModal} onSuccessEdit={onSuccessEdit} inspectingItemID  onDelete={onDeleteInsepctItem}/>
                                            )
                                        }
                                    </React.Fragment>
                                ))}
                            ...
                            <tfoot>
                                <tr >
                                    ...
                                    <td>
                                        <strong>
                                            {inspectItem?.reduce((total, result) => total + parseInt(result.qty || 0, 10), 0)}
                                        </strong>
                                    </td>
                                    <td></td>
                                    <td></td>
                        ...
                        <Stack className='my-2 d-flex justify-content-end'>
                            <Button variant="secondary" size='sm' style={{ width: 'fit-content' }} onClick={() => setShowModalAdd(true)}>Tambah</Button>
                        </Stack>
                    </Card>
                    ...
                    >   
                        ...
                        <Modal.Body>
                            {alertMessage.show && <CustomAlert variants={[alertMessage.success ? 'success' : 'danger']} text={alertMessage.message} />}
                            <Form.Group className="mb-3" controlId="formBasicEmail">
                                <Form.Label className='small-text'><strong>No. Wo</strong></Form.Label>
                                <CustomAsyncSelect
                                    value={{
                                        value: form.wo_id,
                                        label: form.no_wo,
                            ...
                            <Form.Group className="mb-3" controlId="formBasicEmail">
                                <Form.Label className='small-text'><strong>No. Batch</strong></Form.Label>     
                                <Form.Control className='border-bold' type="text" placeholder="No Batch" value={form.no_lot} onChange={(e) => setForm({ ...form, no_lot: e.target.value })} />                            
                            </Form.Group>
                            <Form.Group className="mb-3" controlId="formBasicEmail">
                                <Form.Label className='small-text'><strong>Satuan</strong></Form.Label>     
                                <CustomSelect
                                    options={unitOptions}
                                    value={{
                                        value: form.unit,
                            ...
                            <Form.Group className="mb-3" controlId="formBasicEmail">
                                <Form.Label className='small-text'><strong>No. Meja</strong></Form.Label>     
                                <Form.Control className='border-bold' type="number" placeholder="No Batch" value={form.inspection_table} onChange={(e) => setForm({ ...form, inspection_table: e.target.value })} />                            
                            </Form.Group>

                            {props.jenisProses !== 'mkl-bj' ? (
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Label className='small-text'><strong>Warna</strong></Form.Label>     
                                    <CustomSelect
                                        ...
                                        onChange={({ value,label }) => setForm({ ...form, warna: label, mo_color_id: value })}
                                        placeholder="Pilih"
                                    />                                             
                                </Form.Group>
                            ):(
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    ...
                                    <CustomSelect
                                        ...
                                        onChange={({ value,label }) => setForm({ ...form, color: label, mo_color_id: value })}
                                        placeholder="Pilih"
                                    />                                             
                                </Form.Group>
                            )}
                            ...
                            {props.jenisProses === 'mkl-bj' && (
                                <Form.Group controlId="unit">
                                    ...
                                    <Form.Control
                                        ...
                                        onChange={(e) => setForm({ ...form, jenis_makloon: e.target.value })}
                    ...
                    <ConfirmModal
                        ...
                        title="Konfirmasi"
                    />
                    {showModalAdd &&  <InspectResultAdd closeModal={() => setShowModalAdd(false)} inspectingData={data} jenisProses={props.jenisProses} onSuccessAdd={onSuccessAdd}/>}
                </>
            )}
        </>
    );
